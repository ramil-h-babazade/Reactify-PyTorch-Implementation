...
def DEFAULT_XFORM(s): return s.crop(0, 12).normalize()
...
class NMRSpectrum:
    def __init__(self, path=None, xscale: np.ndarray = None, spectrum: np.ndarray = None, logger=None,):
        ...
        if path:
            ...
            for param in parameters:
                self.param_dict[param.split("= ")[0].strip(" ")] = param.split("= ")[1].strip("\n")
            ...
            self.spectrum = np.fft.fftshift(np.fft.fft(self.fid))[::-1]   # array of intensity values (4878 points) expressed as a complex np array with real & imaginary parts
            self.xscale = self.chemical_shift_scale()                     # chemical shift values (usually between -37.8 ppm and 42.8 ppm) on the x-axis of spectrum
            self.length = len(self.xscale)

    def chemical_shift_scale(self) -> np.ndarray:
        b1_freq = float(self.param_dict["b1Freq"])           # spectrometer's operating frequency in MHz (base frequency)
        dwell_time_us = float(self.param_dict["dwellTime"])  # defined as the number of (micro)seconds between points during data acquisition
        lowest_frequency_hz = float(self.param_dict["lowestFrequency"])
        
        lowest_frequency_ppm = lowest_frequency_hz / b1_freq
        sw_hz = 1e6 / dwell_time_us    # spectral width in Hz (how wide the frequency window is) - spectrum will span sw_hz in total
        sw_ppm = sw_hz / b1_freq       # spectral width in ppm (total range of chemical shifts observed in spectrum)
        highest_frequency_ppm = lowest_frequency_ppm + sw_ppm
        
        # chemical shift scale ranges from lowest_frequency_ppm (~ -37.8 ppm) to highest_frequency_ppm (~ 42.8 ppm), 
        # which is the span of the observed spectrum in chemical shifts.
        return np.linspace( start=lowest_frequency_ppm, stop=highest_frequency_ppm, num=len(self), )
    
    def show(self, imaginary=False, axes=None, **plt_params):
        if not axes: fig, axes = plt.subplots()
        else: fig = axes.get_figure()
        if imaginary: axes.plot(self.xscale, self.spectrum.imag, **plt_params)
        else: axes.plot(self.xscale, self.spectrum.real, **plt_params)
    ...
    def erase(self, low_ppm: float, high_ppm: float, inplace=False):
        '''replace the intensity values between chemical shifts low_ppm and high_ppm with zeroes, thereby erasing that part'''
        s = self if inplace else self.copy()
        selector = (s.xscale > low_ppm) & (s.xscale < high_ppm)
    ...
    def resize(self, length, inplace=False):
        s = self if inplace else self.copy()
        interpolator = interp1d(s.xscale, s.spectrum)             # approximate some function f: s.spectrum = f(s.xscale)
        s.xscale = np.linspace(s.xscale[0], s.xscale[-1], length) # new x-axis with new length within the original boundaries 
        s.spectrum = interpolator(s.xscale)                       # new intensity values in new range found by f 
        s.length = length
        return s
    ...
    def remove_peak(self, peak_ppm: float, window_ppm: float = 0.1, cut: bool = True, inplace=False,  **find_peaks_params,):
        s = self if inplace else self.copy()
        window = (peak_ppm - window_ppm, peak_ppm + window_ppm)
        params = {"height": 0.01 * s.spectrum.real.max(),
            "width": 5,
            "rel_height": 0.995,}
        ...
        if cut: s.cut(s.xscale[lo], s.xscale[hi], inplace=True)
        else:
    ...
    def fit_lorentzian(self, x, y):
        initial = [x, y, 0.07]
        params, pcov = curve_fit(lorentzian, self.xscale, self.spectrum, initial, maxfev=5000)
        return params


    def shift( self, delta: int, inplace=False, circular=False, fill_value: Union[float, np.ndarray] = 0.0, ) -> NMRSpectrum:
        s = self if inplace else self.copy()
        if delta == 0: return self
        if circular: np.roll(s.spectrum, delta)
        elif delta > 0:
    ...
    def __copy__(self): return NMRSpectrum( xscale=self.xscale.copy(), spectrum=self.spectrum.copy(), logger=self.logger )
    ...
    def __rmul__(self, other: Union[NMRSpectrum, float]) -> NMRSpectrum: return self.__mul__(other)
    def __len__(self):    return len(self.spectrum)
    def __getitem__(self, item):  return self.spectrum[item]
    def __setitem__(self, item, value): self.spectrum[item] = value
    
class NMRDataset:
    def __init__(self, dirs, adjust_length=True, dtype="float32", target_length=None, transform: Callable[[NMRSpectrum], NMRSpectrum] = DEFAULT_XFORM, ):
        self.dirs = dirs
        self.spectra = [transform(NMRSpectrum(d)) for d in self.dirs]
        if adjust_length:
            self.min_length = target_length or min(len(s) for s in self.spectra)             # set the min_length as the length of the shortest spectrum
            self.spectra = [ s.resize(self.min_length, inplace=True) for s in self.spectra]  # resize all spectra to have length=min_length
        self.matrix = np.vstack( [s.spectrum.real[: self.min_length] for s in self.spectra]).astype(dtype, casting="same_kind")

    def __iter__(self): return self.spectra.__iter__()
    def __len__(self): return len(self.spectra)
    def __getitem__(self, idx):
        if isinstance(idx, tuple): return self.matrix[idx] # return from matrix 
        else: return self.spectra[idx]  # return from spectra
